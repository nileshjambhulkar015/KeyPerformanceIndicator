...
public interface EmployeeLoginRepo extends JpaRepository<EmployeeLoginEntity, Integer> {
    ...
    @Modifying
    @Query(value = "update employee_login set role_id=:roleId,lst_updt_user_id=:employeeId where emp_eid =:empEId", nativeQuery = true)
    public int updateEmployeeRole(@Param("empEId") String empEId,@Param("roleId") Integer roleId,String employeeId);
    ...
    @Query(value = "select emplog.emp_eid from employee_login emplog where emplog.emp_eid = :userName or emplog.emp_mbno = :userName or emplog.emp_email_id = :userName and emplog.status_cd='A'", nativeQuery = true)
